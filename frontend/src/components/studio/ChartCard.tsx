...
import { Maximize2, Download, Layers, X } from "lucide-react";
import { useStudio } from "@/lib/studio-context";
import { regenerateChartOption, AXIS_TYPES, PIE_TYPES, type SupportedType } from "@/lib/chart-regenerator";
...
export default function ChartCard({ block, index, groupId, isDark, onDrilldown, inlineRows }: ChartCardProps) {
    ...
    const echartsOption = useMemo(() => {
        ...
        // Try to regenerate from filtered rows + meta
    ...
    const cardContent = (
        ...
        >
                ...
                {!isTable && !isKpi && block.meta && (
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition-colors text-muted-foreground hover:text-foreground hover:bg-foreground/10"
                        >
                        ...
                        {showTypePicker && (
                            <div
                                className="absolute right-0 top-full mt-1 z-50 flex flex-col gap-0.5 p-1.5 rounded-xl shadow-xl animate-fadein"
                                style={{ background: "var(--glass-bg)", border: "1px solid var(--glass-border-strong)", backdropFilter: "blur(16px)", minWidth: 110 }}
                            ...
                            >
                                {compatibleTypes.map(ct => (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-[11px] font-medium text-left transition-colors"
                                        style={{
            ...
            {/* ── Content ── */}
            <div className={`flex-1 min-h-0 ${isTable ? "overflow-y-auto p-2" : "p-1"}`}>
                {/* ECharts */}
                {!isTable && !isKpi && echartsOption && (
                    <ReactECharts
                        ...
                        option={{
                            ...
                            dataZoom: currentType !== "pie" && currentType !== "donut" ? [
